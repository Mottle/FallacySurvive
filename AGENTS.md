...
- Deferred register objects (attachments, attributes, effects, nutrition types, data components) are registered on construct events via `MOD_BUS`.
...
- When touching thermal-related survival behavior, verify assumptions against `thermal` engine contracts before changing thresholds or tick cadence.
